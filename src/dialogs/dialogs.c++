...
extern "C" {
  ...
  SDL_Window* dialogs::initImgui() {
    SDL_Window * window;
    uint wf = (SDL_WINDOW_OPENGL | SDL_WINDOW_RESIZABLE | SDL_WINDOW_ALLOW_HIGHDPI);
    
    SDL_WindowFlags window_flags = (SDL_WindowFlags)wf;
    
    window = SDL_CreateWindow("spheres webgl demo", (uint)SDL_WINDOWPOS_CENTERED, (uint)SDL_WINDOWPOS_CENTERED, 1280, 720, window_flags);
  ...
  void dialogs::compose(SDL_Window* window) {
...

    {
      ...
      if (ImGui::Button("Button", ImVec2(0,0) ))                  // Buttons return true when clicked
      { counter++;
      ...
      ImGui::Text("Application average %.3f ms/frame (%.1f FPS)", INIT_FRAMERATE / ImGui::GetIO().Framerate, ImGui::GetIO().Framerate); // NOLINT hicpp-vararg
      ImGui::End();
    }
